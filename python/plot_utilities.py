...
def read_trajectory_solution_file(filename : str, inertial : bool = True):
    """
...
if __name__ == "__main__":
    ...
    for iplot in range(n_plots):
        ...
        if iplot==0:
            ...
            et,x,y,z,rmag = read_trajectory_solution_file(file_to_plot, inertial=False) # rotating frame
            add_line_trajectory_plot(p,x,y,z, scalars = rmag, show_scalar_bar = False, name='rotating')
            plot_body( p, name = 'moon', r = 1737.4, texturemap = os.path.join(PYTHON_DIR,'lroc_color_poles_1k.jpg') )
            if link_views:
                p.link_views()
        else:
            ...
            add_line_trajectory_plot(p,x,y,z, scalars = rmag, show_scalar_bar = True, name='inertial')
        ...
        if iplot==0:
            title = os.path.basename(file_to_plot) + ' [Rotating Frame]'
        else:
        ...
        if iplot==0:
            ...
            h_chart = pv.ChartMPL(f, size=(0.9, 0.25), loc=(0.02, 0.06))
            ...
            f = generate_eclipse_plot_from_json(eclipse_filename,
                                                        ...
                                                        et0 = et[0],
                                                        etf = et[-1])
            h_chart = pv.ChartMPL(f, size=(0.9, 0.25), loc=(0.02, 0.6))
            h_chart.background_color = (1.0, 1.0, 1.0, 0.4)
            p.add_chart(h_chart)
